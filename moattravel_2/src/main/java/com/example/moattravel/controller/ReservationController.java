...
public class ReservationController {
	...
	public String index(@AuthenticationPrincipal UserDetailsImpl userDatailsImpl,
			@PageableDefault(page = 0, size = 10, sort = "id", direction = Direction.ASC) Pageable pageable,
			Model model) {
		User user = userDatailsImpl.getUser();
	...
	public String input(@PathVariable(name = "id") Integer id,
		...
		Integer numberOfPeple = reservationInputForm.getNumberOfPeople();
		Integer capacity = house.getCapacity();

		if (numberOfPeple != null) {
			if (!reservationService.isWithinCapacity(numberOfPeple, capacity)) {
